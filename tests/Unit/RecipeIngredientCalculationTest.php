...
it('scales ingredient totals by recipe servings', function () {
    $user = User::factory()->withCurrentTeam()->create();
    actingAs($user);
    ...
    $gramsItem = collect($calculated)->firstWhere('ingredient.id', $ingGrams->id);
    expect($gramsItem['quantity'])->toBe(500.0)
        ->and($gramsItem['unit'])->toBe(Unit::Grams);

    $piecesItem = collect($calculated)->firstWhere('ingredient.id', $ingPieces->id);
    expect($piecesItem['quantity'])->toBe(5.0)
        ->and($piecesItem['unit'])->toBe(Unit::Pieces);
});
